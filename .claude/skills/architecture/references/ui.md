...
| AmbientLoadingGlow | `app/(app)/AmbientLoadingGlow.tsx` — hidden loading indicator: fixed overlay (`.ambient-loading-glow`) that orbits a second colour around the corner glow while `useNavigationLoadingPending()` is true, held for at least 1600 ms (`MIN_VISIBLE_MS`) after each pending start because cached navigations settle in milliseconds; mounted in the app layout inside NavigationLoadingProvider |
| useTagStateSync | `app/(app)/useTagStateSync.ts` — sidebar tag-path reconciliation (note open, tag edits, palette nav) |
| useSyncDrain | `hooks/useSyncDrain.ts` — the two outbox drain paths: `syncNow` (user-initiated, pushes then pulls, rejects when the outbox is not empty afterwards, or when the pull did not land) and `syncPending` (fired after every mutation, arms the retry loop, never pulls on its own). `useDataSync`'s backoff effect calls the same `drain`, so manual and automatic cannot drift. Also returns `syncFailedState` (re-reads the failed queue, guarded version bump), which `useDataSync` calls from its mount effect — both queue counts are seeded there rather than in the state initializers |
| useServerPull | `hooks/useServerPull.ts` — the one server pull: health gate, `/api/notes` + `/api/todos` merged independently (a failing half must not cost the other its update), then `pullStaleNoteBodies` for the note contents the list cannot carry. Returns `false` when the pull did not fully land — silence here is what made the manual sync button look dead — and `pulledNoteIds`, the ids whose cached body it replaced. Split out of `useDataSync` for the 200-line cap, like `useSyncDrain` |
...
| CommandPaletteNotes | `app/(app)/CommandPaletteNotes.tsx` — the palette's ordinary mode: the note rows, the todo rows and the create row below them. Last on purpose, for the same reason as the tag one: cmdk preselects the first row, so Enter keeps opening the best existing note and the create row inherits the preselection exactly when nothing matched. Unlike a tag path it stays offered even on an exact title hit — two notes may share a title. Split out of `CommandPalette.tsx`, which was at its line cap |
| tagNavigationStore | `app/(app)/tagNavigationStore.ts` — cross-component tag path navigation |
| currentFolderStore | `app/(app)/currentFolderStore.ts` — a mirror of `AppSidebar`'s `folderParent` so the palette's create row files a note into the folder the sidebar is browsing, plus `folderTags`, the shared "root means no tag" rule both create paths use. A mirror, not the owner: the path stays in `useTagStateSync`, which adjusts it during render — legal for `useState`, not for a store write. Only `AppSidebar` calls `set` |
